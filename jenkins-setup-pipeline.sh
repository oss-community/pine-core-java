...
# sonar
echo "Sonar Config"
echo "Login $(cat ./secrets/sonarurlforlocalhost) and keep/change the default credentials to, for example 'admin/password'"
echo "default username: admin"
echo "default password: admin"
SONAR_URL=$(cat ./secrets/sonarurlforlocalhost)
...
# jfrog
echo "JFrog Config"
echo "Login $(cat ./secrets/jfrogartifactorycontexturlforlocalhost) and keep/change the default credentials to, for example 'admin/password'"
echo "Create maven repository with $(cat ./secrets/jfrogartifactoryrepositoryprefix) prefix via Web UI in OSS version."
echo "default username: admin"
echo "default password: password"

ARTIFACTORY_URL="$(cat ./secrets/jfrogartifactorycontexturlforlocalhost)"
...
# nexus
echo "Nexus Config"
echo "Login $(cat ./secrets/nexusartifactoryhosturlforlocalhost) and keep/change the default credentials to, for example 'admin/password'"
...
# jenkins
echo "Jenkins Config"
echo "Login $(cat ./secrets/jenkinsurl) and keep/change the default credentials to, for example 'admin/password'"
echo "Generate first API token via Web UI/Administrator/security"
echo "default username: admin"
JENKINS_DEFAULT_PASSWORD=$(cat ./ci/docker_compose/jenkins/secrets/initialAdminPassword)
...
echo "$JENKINS_USERNAME" > ./secrets/jenkinsuser

echo "Generate first API token via Web UI/Administrator/security"
read -p "Enter token [default: no default]: " jenkinstoken
JENKINS_TOKEN=$jenkinstoken
echo "$JENKINS_TOKEN" > ./secrets/jenkinstoken